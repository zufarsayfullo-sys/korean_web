...
export default function Admin({ session }) {
  ...
  useEffect(() => { fetchAll() }, [])
  useEffect(() => { if (section === 'marks') fetchProgress() }, [section, lessons])

  async function fetchAll() {
    const { data: allProfiles } = await supabase.rpc('get_all_profiles')
    const allP = allProfiles || []
    setPending(allP.filter(p => p.status === 'pending'))
    setActive(allP.filter(p => p.status === 'active'))
    await fetchLessons()
    setLoading(false)
  }
    ...
    <div style={{ minHeight: '100vh', background: '#ECECEA', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <div style={s.layout}>
        ...
        <div style={s.sidebar}>
          ...
          <div style={s.sideSection}>Management</div>
          <div style={{ ...s.sideItem, ...(section === 'attendance' ? s.sideItemActive : {}) }} onClick={() => setSection('attendance')}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#FAFAF8' }}>
                      {['Student', 'Phone', 'Course', 'Actions'].map(h => <th key={h} style={s.th}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {active.map(student => (
            ...
            <div>
              ...
              <div style={s.tableCard}>
                <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <select value={attLesson} onChange={e => { setAttLesson(e.target.value); fetchAttendance(e.target.value) }} style={s.courseSelect}>
                            ...
                            {marksLessons.map(l => {
                              ...
                              return (
                                <td key={l.id} style={{ ...s.td, textAlign: 'center' }}>
                                  <div style={{ width: 32, height: 32, borderRadius: '50%', background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 800, color: textColor, margin: '0 auto' }}>
                                    {pct > 0 ? pct : '—'}
                                  </div>
                                </td>
                              )
